...
ModelGame.MainView = function (){
  ...
  function defineLockInButton(){
    lockInButton = document.querySelector(".lockButton");
    lockInButton.addEventListener("click", handleLockInButton);
  }
  ...
  // Führt den Antwort Check aus
  function handleLockInButton(){
    // Durchsucht alle Frames nach dem Ausgewählten
    var answerFrames = [answerFrame1, answerFrame2, answerFrame3, answerFrame4];
    answerFrames.forEach((element) =>{
      // Gibt den Button-Text zurück
      var buttonValue = element.firstChild.nodeValue;
      // Überprüft welches Feld ausgewählt wurde
      if(element.style.background == "orange"){
          // Bei richtiger Antwort
          if(buttonValue == ModelGame.Categories.currentAnswer){
              console.log("richtig");
              ModelGame.numberOfRightAnswers +=1;
              console.log("Punkte: " + ModelGame.numberOfRightAnswers);
    ...
    // Lässt den Lock In Button nach dem Check verschwinden
